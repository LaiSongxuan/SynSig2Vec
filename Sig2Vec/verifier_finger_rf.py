...
for seed in [111, 222, 333, 444, 555]:
    epoch = str(epoch)
    feats = numpy.load("./exps/seed%d/ebio1_finger/feats_epoch%s.npy"%(seed, epoch))
    
    # Use training set statistics for normalization. This is important for the finger scenario.
    featsTrain = numpy.load("./exps/training/seed%d/ebio1_finger/feats_epoch%s.npy"%(seed, epoch))
    feats = feats - numpy.mean(featsTrain, axis=0)
    feats = feats / numpy.std(featsTrain, axis=0)

    feats = feats.reshape(feats.shape[0], -1, 32)
    feats = feats / (numpy.sum(feats**2, axis=-1, keepdims=True)**0.5 + 1e-8)
    feats = feats.reshape(feats.shape[0], -1)
    feats = feats / (numpy.sum(feats**2, axis=1, keepdims=True)**0.5 + 1e-8)
    labels = numpy.load("./exps/seed%d/ebio1_finger/labels_epoch%s.npy"%(seed, epoch))
    inds = numpy.load("./exps/seed%d/ebio1_finger/inds_epoch%s.npy"%(seed, epoch))

    N_samples = feats.shape[0] // N_user
    for rand in range(N_loops):
        EERs = []; datum_p = []; datum_n = []
        rf = feats[0::N_samples]
        for k in numpy.arange(0, N_user): 
            idxUser = numpy.where(labels==k)[0]
            dset = feats[idxUser]; ind = inds[idxUser]
            
            deviceIndP = numpy.array([0,1,4,5,8,9,12,13]) + device * 2
            deviceIndN = numpy.array([16,17,18,22,23,24]) + device * 3
            ...
            if N_train == 1:
                ...
                temp = dset[idxTemp]
            # Genuine signatures from other users (one sample per user) are used as random forgeries.
            testG = dset[list(set(idxGen) - set(idxTemp))]
            testRF = numpy.delete(rf, k, axis=0)
            test = numpy.concatenate((testG, testRF), axis=0)
            testInd = numpy.zeros(test.shape[0])
            testInd[:testG.shape[0]] = 1
...
for seed in [111, 222, 333, 444, 555]:
    epoch = str(epoch)
    feats = numpy.load("./exps/seed%d/ebio2_finger/feats_epoch%s.npy"%(seed, epoch))
    
    # Use training set statistics for normalization. This is important for the finger scenario.
    featsTrain = numpy.load("./exps/training/seed%d/ebio2_finger/feats_epoch%s.npy"%(seed, epoch))
    feats = feats - numpy.mean(featsTrain, axis=0)
    feats = feats / numpy.std(featsTrain, axis=0)
    ...
    feats = feats.reshape(feats.shape[0], -1)
    feats = feats / (numpy.sum(feats**2, axis=1, keepdims=True)**0.5 + 1e-8)
    labels = numpy.load("./exps/seed%d/ebio2_finger/labels_epoch%s.npy"%(seed, epoch))
    inds = numpy.load("./exps/seed%d/ebio2_finger/inds_epoch%s.npy"%(seed, epoch))
    ...
    for rand in range(N_loops):
        ...
        for k in numpy.arange(0, N_user): 
            ...
            if device == 0:
                deviceIndP = numpy.array([0,1,4,5,8,9,10,11])
                deviceIndN = numpy.array([16,17,18,22,23,24])
            else: #device == 1
                deviceIndP = numpy.array([2,3,6,7,12,13,14,15])
                deviceIndN = numpy.array([19,20,21,25,26,27])
            dset = numpy.concatenate((dset[deviceIndP], dset[deviceIndN]), axis=0)
            ind = numpy.concatenate((ind[deviceIndP], ind[deviceIndN]), axis=0)
