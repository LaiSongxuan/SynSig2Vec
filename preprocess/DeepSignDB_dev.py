...
finger = False
if finger:
	path = "Path_To_DeepSignDB/DeepSignDB/Development/finger"
else:
	path = "Path_To_DeepSignDB/DeepSignDB/Development/stylus"
usersMCYT = {}
usersBSID = {}
usersBSDB2 = {}
usersEBio1 = {}
...
if finger:
	...
	if len(usersEBio1) > 0:			
		output = open('../data/EBio1_dev_finger.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:			
		output = open('../data/EBio2_dev_finger.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()
elif virtualPenup:
	if len(usersMCYT) > 0:
		output = open('../data/MCYT_dev_pad.pkl', 'wb')
		pickle.dump(usersMCYT, output)
		output.close()

	if len(usersBSID) > 0:
		output = open('../data/BSID_dev_pad.pkl', 'wb')
		pickle.dump(usersBSID, output)
		output.close()

	if len(usersBSDB2) > 0:
		output = open('../data/BSDB2_dev_pad.pkl', 'wb')
		pickle.dump(usersBSDB2, output)
		output.close()
				
	if len(usersEBio1) > 0:			
		output = open('../data/EBio1_dev_pad.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:
		output = open('../data/EBio2_dev_pad.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()
else:
	if len(usersMCYT) > 0:
		output = open('../data/MCYT_dev.pkl', 'wb')
		pickle.dump(usersMCYT, output)
		output.close()

	if len(usersBSID) > 0:
		output = open('../data/BSID_dev.pkl', 'wb')
		pickle.dump(usersBSID, output)
		output.close()

	if len(usersBSDB2) > 0:
		output = open('../data/BSDB2_dev.pkl', 'wb')
		pickle.dump(usersBSDB2, output)
		output.close()
				
	if len(usersEBio1) > 0:			
		output = open('../data/EBio1_dev.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:
		output = open('../data/EBio2_dev.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()